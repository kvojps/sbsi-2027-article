...
- **Bibliografia:** `artigo/referencias.bib`. Não criar entradas novas sem antes conferir se já
  existe uma com a mesma obra.
- **Scaffold LaTeX:** `artigo/artigo.tex`. Os nomes de seção aqui e lá são os mesmos, na mesma ordem.
...
(`initexmf --set-config-value "[MPM]AutoInstall=1"`). De dentro de `artigo/`:

```
pdflatex artigo && bibtex artigo && pdflatex artigo && pdflatex artigo
```

O `sbc-template.sty` e o `sbc.bst` estão copiados neste diretório de propósito: assim `artigo/` sobe
para o Overleaf inteiro, sem depender de `template-latex/`.

A conferência automática do andaime é `python scripts/verificar_andaime.py`. Ela lê o esqueleto, o
...
Título, abstract estruturado em inglês (texto congelado em `submissao/jems3-pacote-registro.md`,
...
Geradas por BibTeX de `artigo/referencias.bib` com o estilo `sbc`. Precisam estar presentes, pela
