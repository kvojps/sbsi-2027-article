...
e completas, e acompanha as três dimensões do modelo de referência. Todas as classes chegam
definidas, o que o artefato analisado não fazia: 24 definições vêm do glossário do apêndice da
conceituação — e é o que sustenta afirmar que já existiam —, cinco foram redigidas a partir do papel
que o conceito exerce nos diagramas, e quinze nasceram com os conceitos que as rodadas introduziram.
...
`Organization`, que pode afetar os projetos observados ou ser afetada por eles. `StakeHolder` é
«roleMixin», porque a definição de parte interessada abrange indivíduo, grupo e organização — três
princípios de identidade — e nenhum sortal único a comporta. `System`, o ator não-humano que troca
dados com o observatório, é papel de `ComputationalSystem`, o tipo que lhe dá identidade e que a
formalização deixara implícito; `ObservatoryGroup` é «collective».
...
gerenciamento ao projeto por cujos dados ele responde; `ViewProvision`, um gerenciamento à interface
que ele disponibiliza; `SocialInteraction` e `Log`, um agente à interface com que ele interage e à
que registra sua ação; e `SoftwareExecution`, `ServiceProvision` e `Connection` cobrem execução,
prestação de serviço e conexão à rede.
...
[almeida2019gufo], executada pela `ontouml-js` — a mesma biblioteca do verificador de conformidade
—, e cada elemento da OWL corresponde ao elemento do modelo que o gerou: é essa correspondência a
garantia de preservação semântica que uma tradução manual não oferece.
...
a cada execução. A **C1** declara localmente os termos da gUFO que o artefato referencia, copiando
da ontologia de fundamentação o tipo, o rótulo, o domínio e o alcance de cada um, para que o arquivo
se sustente quando a importação não é resolvida. A **C2** devolve a cada classe sua definição e seus
rótulos preferidos. A **C3** declara disjuntos os «kind» do modelo — consequência da semântica do
estereótipo, que a transformação não carrega — e os três subtipos de `Software`. A **C4** dá nome à
inversa de cada propriedade de objeto, sem o que a proveniência teria de ser percorrida por caminho
invertido a cada passo. A **C5** acrescenta título, licença e versão. Somam 648 triplas, publicadas
à parte como o acréscimo exato, ao lado dos dois artefatos, para que qualquer terceiro reconstitua a
soma.

Três decisões foram deliberadamente **não** tomadas, com a razão registrada: os identificadores não
foram renomeados, porque isso quebraria a correspondência com o modelo; os papéis irmãos não foram
declarados disjuntos, porque nada na fonte impede que um componente colete e armazene; e nenhuma
axiomatização pesada foi acrescentada, por decisão de escopo (§8).

## 6.3. Métricas do artefato

A Tabela 2 conta o que está nos arquivos, sem a gUFO importada: sob avaliação está a ontologia de
domínio.
...
ao diagrama (§8). O salto nas **anotações** é a C2 medida: a distância entre um artefato cujos
termos o leitor adivinha e um em que cada um se explica.
